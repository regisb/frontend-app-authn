...
const UsernameField = (props) => {
  ...
  // console.log("I was here", usernameSuggestions.length, errorMessage);
  if (usernameSuggestions.length > 0 && errorMessage) {
    suggestedUsernameDiv = suggestedUsernames();
  } else if (usernameSuggestions.length > 0 && props.value === ' ') {
    className = 'suggested-username';
    iconButton = <IconButton src={Close} iconAs={Icon} alt="Close" onClick={(e) => props.handleUsernameSuggestionClose(e)} variant="black" size="sm" className="suggested-username-close-button" />;
    suggestedUsernameDiv = suggestedUsernames();
  }
  return (
    <FormGroup {...props}>
